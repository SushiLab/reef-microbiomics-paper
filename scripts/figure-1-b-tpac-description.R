...
metagenomes_tpac_coral<-read_sheet("https://docs.google.com/spreadsheets/d/1ukivQCmnq6PePM5vl6r3PdRJXSwZ_iChEN1LRbGD0EQ/edit#gid=0",sheet="Sheet 1") %>%
  separate(sample,into=c(NA,"biosample",NA),sep="_",remove=FALSE) %>%
  mutate(biome_group=word(type,2))

metagenomes_tpac_water<-read_sheet("https://docs.google.com/spreadsheets/d/1ukivQCmnq6PePM5vl6r3PdRJXSwZ_iChEN1LRbGD0EQ/edit#gid=0",sheet="Sheet 2") %>%
  separate(sample,into=c(NA,"biosample",NA),sep="_",remove=FALSE)
